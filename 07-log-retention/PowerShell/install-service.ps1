# ============================================================
# INSTALL LOG RETENTION AS WINDOWS SERVICE
# ============================================================

param(
    [string]$NSSMPath = ".\nssm.exe",
    [string]$ScriptPath = ".\log-retention.ps1",
    [string]$ServiceName = "LogRetention",
    [string]$DisplayName = "Log Retention Service",
    [string]$Description = "Automatically manages log files based on retention policies"
)
...
Write-Host "  cmdkey /add:LOG_RETENTION_EMAIL_CRED /user:your-email@domain.com /pass:YourPassword" -ForegroundColor White

Write-Host "`nTest Mode:" -ForegroundColor Cyan
Write-Host "  To test without deleting files, set in config.ps1:" -ForegroundColor White
Write-Host "  `$DryRunMode = `$true" -ForegroundColor White