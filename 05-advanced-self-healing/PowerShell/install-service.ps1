# ============================================================
# INSTALL ADVANCED SELF-HEALING AS WINDOWS SERVICE
# ============================================================

param(
    [string]$NSSMPath = ".\nssm.exe",
    [string]$ScriptPath = ".\advanced-selfhealing.ps1",
    [string]$ServiceName = "AdvancedSelfHealing",
    [string]$DisplayName = "Advanced Self-Healing Service",
    [string]$Description = "Monitors database conditions and triggers automated actions"
)
...
Write-Host "`nCredential Setup:" -ForegroundColor Cyan
Write-Host "  For Oracle: cmdkey /add:DB_ORACLE_CRED /user:username /pass:password" -ForegroundColor White
Write-Host "  For SQL Server: cmdkey /add:DB_SQLSERVER_CRED /user:username /pass:password" -ForegroundColor White
Write-Host "  For MySQL: cmdkey /add:DB_MYSQL_CRED /user:username /pass:password" -ForegroundColor White
Write-Host "  For PostgreSQL: cmdkey /add:DB_POSTGRESQL_CRED /user:username /pass:password" -ForegroundColor White