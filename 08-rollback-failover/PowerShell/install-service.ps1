# ============================================================
# INSTALL ROLLBACK & FAILOVER AS WINDOWS SERVICE
# ============================================================

param(
    [string]$NSSMPath = ".\nssm.exe",
    [string]$ScriptPath = ".\rollback-failover.ps1",
    [string]$ServiceName = "RollbackFailover",
    [string]$DisplayName = "Rollback & Failover Service",
    [string]$Description = "Automatically rolls back or fails over deployments when failures are detected"
)
...
& $NSSMPath set $ServiceName AppDirectory $ScriptDir# Set failure recovery
...
Write-Host "`nEmail Credential Setup:" -ForegroundColor Cyan
Write-Host "  cmdkey /add:ROLLBACK_FAILOVER_EMAIL_CRED /user:your-email@domain.com /pass:YourPassword" -ForegroundColor White

Write-Host "`nRollback Scripts Location:" -ForegroundColor Cyan
Write-Host "  $ScriptDir\rollback-scripts\" -ForegroundColor White
Write-Host "  Place your custom rollback and failover scripts here." -ForegroundColor White